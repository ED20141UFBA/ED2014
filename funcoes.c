...
void Menu(Processo *Pilha)                              //Menu e chamada para as funções
{
	...
	while(1)
	{
        ...
        printf(" %c                                  %c                             %c\n",186,186,186);
        printf(" %c 0 - Inserir novo Processo        %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_Insere)/CLOCKS_PER_SEC),186);
        printf(" %c 1 - Apagar Ultimo Processo       %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_Apaga)/CLOCKS_PER_SEC),186);
        printf(" %c 2 - Apagar um Processo pelo ID   %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_ApagaID)/CLOCKS_PER_SEC),186);
        printf(" %c 3 - Exibir Pilha de Processos    %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_ExibeTudo)/CLOCKS_PER_SEC),186);
        printf(" %c 4 - Exibir por ID                %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_ExibeID)/CLOCKS_PER_SEC),186);
        printf(" %c 5 - Exibir proximo Processo      %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_ExibeProximo)/CLOCKS_PER_SEC),186);
        printf(" %c 6 - Exibir ultimo Processo       %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_ExibeUltimo)/CLOCKS_PER_SEC),186);
        printf(" %c 7 - Tamanho da Pilha             %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_ExibeTamanho)/CLOCKS_PER_SEC),186);
        printf(" %c 8 - Limpar Pilha                 %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_LimpaTudo)/CLOCKS_PER_SEC),186);
        printf(" %c 9 - Organiza por Nome de Mercado %c %c Tempo Execursao: %f %c\n",186,186,26, ((double)(Clock_OrganizaPorNome)/CLOCKS_PER_SEC),186);
        printf(" %c                                  %c                             %c\n",186,186,186);
...
void ExibirID(Processo *Pilha)                          // Exibe somente o processo com a ID selecionada
{
    ...
    if(Pilha->prox != NULL)
    {
        while(Pos->prox != NULL)            //Percorre a Pilha Principal até o ultimo elemento apontar para NULL.
        {
            if(LocalizarEsteID != Pos->ID)      // Se entrar aqui é pq é diferente, então avança os nós da Pilha principal.
            {
                Ant = Ant->prox;
                Pos = Pos->prox;
            }
...
void libera(Processo *Pilha)                            //Libera todos os espaço alocado da Pilha
{
    Clock_LimpaTudo=0;
...
void EmpilharProcesso(Processo *Pilha)                  //Iserção de nós na pilha
{
    Clock_Insere=0;
    float Clock_Pause=0;
    Clock_Pause = clock();
...
void OrganizaProcessoPorNomeMercado(Processo *Pilha)    //Verifica os processos de um mercado para organizar no TOPO da Pilha
{
    ...
    if(!teste_vazia(Pilha))
    {
        while(Pos->prox != NULL)            //Percorre a Pilha Principal até o ultimo elemento apontar para NULL.
        {
            if(stricmp(Mercado,Pos->NomeMercado) != 0)      // Se entrar aqui é pq é diferente, então avança os nós da Pilha principal.
            {
                Ant = Ant->prox;
                Pos = Pos->prox;
            }
            else                            //Entrando aqui é pq é Igual, então ele irá passar o novo elemento para a pilhaaux e retirar da Pilha Principal.
            {
                NovoElementoAux = Pos;
                if(Pos->prox != NULL)           //Verifica se o próximo aponta para NULL para avança com Pos e Ant;
                {
                    Pos = Pos->prox;
                    Ant->prox = Pos;
                }
        ...
        if(stricmp(Mercado,Pos->NomeMercado) == 0)      //Resolve o ultimo nó;
        {
            while(CorrePilhaAux->prox != NULL)
                CorrePilhaAux = CorrePilhaAux->prox;
...
void ApagarID(Processo *Pilha)                          //Apaga um processo com o ID informado.
{
    ...
    if(Pilha->prox != NULL)
    {
        while(Pos->prox != NULL)            //Percorre a Pilha Principal até o ultimo elemento apontar para NULL. Ou seja, ele compara até o penultimo ítem,
        {
            if(ApagarEsteID != Pos->ID)      // Se entrar aqui é pq é diferente, então avança os nós da Pilha principal.
            {
                Ant = Ant->prox;
                Pos = Pos->prox;
            }
        ...
        if(ApagarEsteID == Pos->ID)     // Verifica se o No procurado é o ultimo
        {
            LiberaNo = Pos;
            Ant->prox = NULL;
            free(LiberaNo);
...
void Apagar(Processo *Pilha)                            //Apaga um nó da pilha. O ultimo elemento inserido.
{
    Clock_Apaga = 0;
    Clock_Apaga = clock();
